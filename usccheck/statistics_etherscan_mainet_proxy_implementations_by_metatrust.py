...
FLAG_DOWNLOAD_CODE = True
workdir = "usccheck/OnchainContractData/proxy_implementations"
...
proxy_contracts = set([item["proxy"] for item in multiple_non_zero_impls])
impl_contracts = set()
for item in multiple_non_zero_impls:
    impl_contracts.update(item["implementations"])
print("proxies with multiple non-zero implementation: ", len(multiple_non_zero_impls),
      sum([len(item["implementations"]) for item in multiple_non_zero_impls]))
print(">>> proxy contracts:", len(proxy_contracts))
print(">> implementation contracts:", len(impl_contracts))
print(">> proxy\/implementation contracts:",
      len(proxy_contracts.union(impl_contracts)))
print(">> proxy/\implementation contracts:",
      len(proxy_contracts.intersection(impl_contracts)))
...
json.dump(dict(uninitialized_impls=uninitialized_impls,
          ...
          open(os.path.join(workdir, "etherscan_mainnet.statistics.json"), "w"),
          indent=4
          )
if FLAG_DOWNLOAD_CODE:
    category_dir = os.path.join(
        workdir, "category/ethereum_mainnet/multi_non_zero_impl")
    ...
    """
    category_dir = os.path.join(workdir, "category/ethereum_mainnet/sole_non_zero_impl")
    if not os.path.exists(category_dir):
        os.makedirs(category_dir)
    ...
    category_dir = os.path.join(workdir, "category/ethereum_mainnet/uninitialized_impl")
    if not os.path.exists(category_dir):
        os.makedirs(category_dir)
