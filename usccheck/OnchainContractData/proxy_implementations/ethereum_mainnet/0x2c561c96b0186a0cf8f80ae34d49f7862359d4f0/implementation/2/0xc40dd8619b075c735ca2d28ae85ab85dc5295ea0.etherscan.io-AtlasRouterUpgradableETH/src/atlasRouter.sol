...
contract AtlasRouterUpgradableETH is Initializable{
    ...
    function initialize(address _routerAddress,address _factory,address weth,address dai,address usdt,address usdc,address _owner,address _feeCollector,uint256 _fees)external initializer{
    ...
    function swapExactTokensForTokens(  
        ...
        uint deadline) external payable{
            ...
            if(IERC20(path[0]).allowance(address(this),uniswapV2) < amountInForSwap){
                TransferHelper.safeApprove(path[0],uniswapV2,0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0],msg.sender,address(this),amountInForSwap);
        TransferHelper.safeTransferETH(feeCollector,feesInEth);
        uint256 receivedAmt = IERC20(path[0]).balanceOf(address(this));
        IUniswapV2Router02(uniswapV2).swapExactTokensForTokensSupportingFeeOnTransferTokens(receivedAmt,amountOutMin,path,to,deadline);
        refundDustEth();
    }
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferETH(feeCollector,feesInEth);
        uint256 receivedAmt = IERC20(path[0]).balanceOf(address(this));
        IUniswapV2Router02(uniswapV2).swapExactTokensForETHSupportingFeeOnTransferTokens(receivedAmt,amountOutMin,path,to,deadline);
        refundDustEth();
    }
