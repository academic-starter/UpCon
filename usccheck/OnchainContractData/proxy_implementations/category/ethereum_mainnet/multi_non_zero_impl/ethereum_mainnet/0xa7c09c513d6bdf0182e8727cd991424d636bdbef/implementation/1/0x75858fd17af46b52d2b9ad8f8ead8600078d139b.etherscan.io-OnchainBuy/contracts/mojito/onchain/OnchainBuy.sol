 ...
 */
contract OnchainBuy is Initializable, ReentrancyGuard, AdminControlUpgradeable{
    ...
    // validating saleId
    mapping(string => bool) public usedSaleId;
    ...
    function createOrUpdateSale(
        ...
        if (list.paymentStatus == PaymentStatus.fiat) {
            require(
                list.minimumFiatPrice != 0 && list.paymentCurrency.length == 1,
                "Invalid Fiat Price Details"
            );
        } else {
        ...
        if (list.transactionStatus == TransactionStatus.mint) {
            ...
            if (!isAdmin(msg.sender)) {
                require(
                    list.nftStartTokenId == list.nftEndTokenId &&
                        list.nftStartTokenId != 0,
                    "both ids should be the same"
                );
                require(
                    ownerCheck(list.nftContractAddress, list.nftStartTokenId),
    ...
    function buy(
        ...
        if (tax > 0) {
            _handlePayment(
                list.buyer,
                saleList.paymentSettlement.taxSettlementAddress,
                list.paymentToken,
                tax
            );
        }
    ...
    function errorHandling(
        ...
        if (saleList.paymentStatus == PaymentStatus.fiat) {
            require(
                saleList.paymentCurrency[0] == _paymentToken,
        ...
        } else if (saleList.transactionStatus == TransactionStatus.transfer && _quantity >= 1) {
            _minimumPrice = (_minimumPrice * _quantity);
        }
    ...
    function _tokenTransaction(
        ...
        if (IERC165(_tokenContract).supportsInterface(ERC721_INTERFACE_ID)) {
            if (_status == TransactionStatus.transfer) {
                ...
                require(
                    tokenUris.length == _tokenQuantity,
                    "token uri should be equal to mint quantity"
                );
                // Minting the ERC721 in a batch
                nftTokenId = IERC721CreatorCore(_tokenContract)
                    .mintExtensionBatch(_buyer, tokenUris);
            ...
            if (_status == TransactionStatus.transfer) {
                ...
                if (
                    ...
                    nftTokenId = IERC1155CreatorCore(_tokenContract)
                        .mintExtensionNew(to, amounts, tokenUris);
    ...
    function _handlePayment(
        ...
        bool success;
        if (!Address.isContract(_paymentToken)) {
    ...
    function ownerCheck(
        ...
        if (
            IERC165(nftContractAddress).supportsInterface(ERC721_INTERFACE_ID)
        ) {
            require(
            ...
            uint256 ownerBalance = IERC1155(nftContractAddress).balanceOf(
                msg.sender,
                tokenId
            );
            require(
    ...
    function cancelSale(string memory saleId) external {
        require(
            usedSaleId[saleId],
        ...
        if (!isAdmin(msg.sender)) {
            require(
                ownerCheck(
                    listings[saleId].nftContractAddress,
                    listings[saleId].nftStartTokenId
                ),
                "should be token Owner"
            );
        }
